...
$member_fname = filter_input(INPUT_POST, 'member_fname');
...
$item = array();
$item['member_fname'] = $member_fname;
$item['message'] = '';
$item['error'] = FALSE;
...
$item = array();
$item['member_lname'] = $member_lname;
$item['message'] = '';
$item['error'] = FALSE;
...
function add_member($member_email, $member_fname, $member_lname,
    ...
    $query = '
        ...
        VALUES (:member_email, :password, :member_fname, :member_lname)';
    $statement = $db->prepare($query);
    $statement->bindValue(':member_email', $member_email);
...
function update_member_temp($forgot_password_temp, $member_email, $expiration_time){
	...
	$query = '
	UPDATE members
	SET member_tmp_pswd = :forgot_password_temp, pswd_timeout = :expiration_time
	WHERE member_email = :member_email';
...
function is_valid_member_login($member_email, $member_password1) {
    ...
    $query = '
        SELECT * FROM members
        WHERE member_email = :member_email AND member_password = :password';
    $statement = $db->prepare($query);
    $statement->bindValue(':member_email', $member_email);
    $statement->bindValue(':password', $password);
    $statement->execute();
...
##can't use the othe pwvalidate, so must make new one for forgot password functionality" this is just the beginning to check that it works 10/28/17
...
function update_member_pw($member_password1, $member_email) {
